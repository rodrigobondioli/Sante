...
import type { BarResumo, RiskAlert } from "@/lib/admin/queries";
...
function RiskChips({ alertas }: { alertas: RiskAlert[] }) {
  if (!alertas.length)
    return (
      <span
        style={{
  ...
  const top = alertas.slice(0, 2);
  const rest = alertas.length - top.length;
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
      ...
      {rest > 0 && (
        <span style={{ fontSize: 10, color: "var(--fg-subtle)" }}>+{rest}</span>
      )}
    </div>
  );
}

function CoverageBar({ pct, total }: { pct: number; total: number }) {
  if (total === 0) return <span style={{ fontSize: 12, color: "var(--fg-subtle)" }}>—</span>;
...
function Activity7d({
  ...
  const hasData = turnos > 0 || comandas > 0 || faturamento > 0;
  if (!hasData)
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 860 }}>
          <thead>
            <tr>
              ...
              <th style={{ ...thStyle, textAlign: "right", paddingRight: 20 }}></th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((bar) => {
              const isHovered = hover === bar.id;
              const hasRed = bar.alertas.some((a) => a.level === "red");
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)" }}>
                    <Activity7d
                      turnos={bar.turnos_7d}
                      comandas={bar.comandas_7d}
                      faturamento={bar.faturamento_7d}
                    />
                  </td>

                  {/* Cobertura */}
                  <td style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)" }}>
                    <CoverageBar pct={bar.cobertura_custo_pct} total={bar.total_produtos} />
                  </td>

                  {/* Alertas */}
                  <td style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)" }}>
                    <RiskChips alertas={bar.alertas} />
                  </td>
                  ...
                  >
                    <span
                      style={{
                        fontSize: 12,
                        color: isHovered ? "var(--fg)" : "var(--fg-subtle)",
                    ...
                    >
                      ...
                      <svg
                        ...
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        style={{ transform: isHovered ? "translateX(2px)" : "none", transition: "transform 120ms" }}
                      >
...
// hash simples para cor do avatar
